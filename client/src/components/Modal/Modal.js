...
const Modal = ({
  ...
  const next30min = new Date(
    Math.ceil(currentDate.getTime() / interval30) * interval30
  );
  const next90min = new Date(
    Math.ceil(currentDate.getTime() / interval30) * interval30
  ).setMinutes(next30min.getMinutes() + 90);

  const [location, setLocation] = useState("");
  const [activityType, setActivityType] = useState("");
  const [startDate, setStartDate] = useState(
    next30min.getHours() < 21 && next30min.getHours() >= 8
      ? next30min
      : setHours(setMinutes(tomorrow.setDate(tomorrow.getDate() + 1), 0), 8)
  );
  const [endDate, setEndDate] = useState(
    next30min.getHours() < 21 && next30min.getHours() >= 8
      ? next90min
      : setHours(setMinutes(tomorrow, 30), 9)
  );
  ...
  const reset = () => {
    ...
    setStartDate(
      next30min.getHours() < 21 && next30min.getHours() >= 8
        ? next30min
        : setHours(setMinutes(tomorrow, 0), 8)
    );
    setEndDate(
      next30min.getHours() < 21 && next30min.getHours() >= 8
        ? next90min
  ...
  const makeBooking = async (e) => {
    ...
    if (
      ...
      if (activityType === "" || activityType === "DEFAULT") {
        document
          .querySelector(".label-activity-type")
      ...
      <div className="modal">
        <form onSubmit={makeBooking} ref={domNode}>
          <div className="form-container">
            ...
            <div className="item">
              <label className="label-location" htmlFor="location">
                Location *
              </label>
              <select
            ...
            <div className="item">
              <label className="label-activity-type" htmlFor="activity-type">
                Activity Type *
              </label>
              <select
            ...
            <div className="item">
              <label className="label-start-date" htmlFor="start-time">
                Start *
              </label>
              <DatePicker
            ...
            <div className="item">
              <label className="label-end-date" htmlFor="end-time">
                End *
              </label>
              <DatePicker
                placeholderText="Select"
            ...
            <div className="item">
              <label className="label-aircraft" htmlFor="aircraft">
                Aircraft *
              </label>
              <select
            ...
            <div className="item" id="div-instructor">
              <label className="label-instructor" htmlFor="instructor">
                Instructor *
              </label>
              <select
            ...
            <div className="item">
              ...
              <div className="flight-type">
                <input
                  ...
                  onClick={() => {
                    setFlightType("Local");
                    const labelFlightType =
                      document.querySelectorAll(".label-flight-type");
                    for (let i = 0; i < labelFlightType.length; i++) {
                  ...
                  name="flightType"
                />
                <label className="label-flight-type" htmlFor="local">
                  Local
                </label>
                <input
                  type="radio"
                  ...
                  name="flightType"
                />
                <label className="label-flight-type" htmlFor="cross-country">
                  Cross Country
                </label>
              </div>
            </div>
